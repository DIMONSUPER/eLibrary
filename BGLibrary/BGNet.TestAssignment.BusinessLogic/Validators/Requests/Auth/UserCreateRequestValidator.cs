...
public class UserCreateRequestValidator : AbstractValidator<FullUserDto>
{
    public UserCreateRequestValidator(IStringLocalizer<ValidationMessages> localizer)
    {
        ...
        RuleFor(x => x.Password).MinimumLength(6).WithMessage(x => localizer[nameof(ValidationMessages.UserPasswordMinimumLength)]);
        RuleFor(x => x.Password).MaximumLength(16).WithMessage(x => localizer[nameof(ValidationMessages.UserPasswordMaximumLength)]);
